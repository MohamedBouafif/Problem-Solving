...
void solve()
{
    int n;
    cin>>n;
    vi a(n), prefgcd(n+1,0), sufgcd(n+1,0);
    for(int i = 0; i < n;i++)cin>>a[i];
    for(int i = 1; i <= n; i++)prefgcd[i] = gcd(a[i-1] , prefgcd[i-1]);
    for(int i = n ; i>=0 ;i--)sufgcd[i] = gcd(sufgcd[i+1], a[i-1]);
    int ans = 0;
    for(int i = 1; i <= n; i++)ans = max(ans, gcd(prefgcd[i-1], sufgcd[i+1]));
    cout<<ans<<endl;
}
